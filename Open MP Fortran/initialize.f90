 ...
 !  COMPUTATIONAL REGION will be: [-26, 26] x [-26, 26],  with h = 0.1:
    ...
    INTEGER  L ! side of the square: L = h*min(abs(M1),abs(M2),abs(N1),abs(N2))
    REAL*8,  PARAMETER ::  re_fv1 = 0.5D0  ! 50% de L
    REAL*8,  PARAMETER ::  re_fv2 = 0.7D0  ! 70% de L
    REAL*8,  PARAMETER ::  re_fv3 = 0.9D0  ! 90% de L

    REAL*8   refv1  ! refv1 = L * re_fv1 <--- main position of far field ZONE 1
    REAL*8   refv2  ! refv2 = L * re_fv2 <--- main position of far field ZONE 2
    REAL*8   refv3  ! refv3 = L * re_fv3 <--- main position of far field ZONE 3
 ...
 !******************************************************************
    ...
    L = h*min(abs(M1),abs(M2),abs(N1),abs(N2))
    refv1 = L*re_fv1 !<--- main position of far field ZONE 1
    refv2 = L*re_fv2 !<--- main position of far field ZONE 2
    refv3 = L*re_fv3 !<--- main position of far field ZONE 3

    x_min = M1*h; x_max = M2*h;
    y_min = N1*h; y_max = N2*h;

    if ( max(refv1, refv2, refv3) > min( abs(x_min), abs(x_max), abs(y_min), abs(y_max) ) ) &
    then
 ...
 !--------------------------------------------------------------
    ...
    y1 = +1.0D0;   ! <--- y-coordinate of 1st singularity

    x2 = -1.5D0;   ! <--- x-coordinate of 2nd singularity
    y2 = +0.5D0;   ! <--- y-coordinate of 2nd singularity

    x3 = +0.0D0;   ! <--- x-coordinate of 3rd singularity
    y3 = -1.5D0;   ! <--- y-coordinate of 3rd singularity

    x4 = -2.0D0;   ! <--- x-coordinate of 4th singularity
    y4 = -0.5D0;   ! <--- y-coordinate of 4th singularity
    ...
    b =  3.0D0;   ! expected far-field solution value is:
                                  ! (b1 + b2 + b3 + b4)/4
    expected_b = (b1 + b2 + b3 + b4)/4D0;  ! <--- this value is: 2.5
