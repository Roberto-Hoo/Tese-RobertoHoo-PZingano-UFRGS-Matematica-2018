...
PROGRAM u24h_omp10    ! <--- to be executed ONLY after program
  ...
  do numLoops = 1, 2000
    ...
    if ( tF <  0.6D0 ) tF_new =   1D0;  ! <------ eliminate later
    if ( tF <  0.1D0 ) tF_new =  0.5D0;  ! <------ eliminate later
    ...
    DO WHILE ( t < tF_check )
      ...
      DO external_iteration_count = 1, no_external_iterations
        ...
        i0 = M1 + tid*main_chunk_size; ! Cada thread tem seu próprio i0
        iK = i0 + main_chunk_size;     ! Cada iK tem seu próprio iK
        ...
        if ( tid > 0 .AND. tid < Nthreads-1 ) then
          ...
          ENDDO ! DO jc = 1, omp_iterations

        endif ! if ( tid > 0 .AND. tid < Nthreads-1 ) then
        ...
        endif ! if ( tid.EQ.Nthreads-1 ) then
        ...
        t = t + omp_time_advance;! omp_time_advance = omp_iterations*dt =1/200

        !       write(6,*) ' t = ', real(t)

      ENDDO !DO external_iteration_count = 1, no_external_iterations
    ...
    write(70,*) "expected value of b: ", (b1 + b2 + b3 + b4)/4D0
    write(70,*)
