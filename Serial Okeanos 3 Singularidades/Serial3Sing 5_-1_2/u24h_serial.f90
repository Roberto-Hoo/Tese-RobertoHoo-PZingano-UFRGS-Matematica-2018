...
REAL*8,  DIMENSION(:), ALLOCATABLE ::  media_du_sobre_dt
REAL*8,  DIMENSION(:), ALLOCATABLE ::  sup_du_sobre_dt
...
INTEGER  no_runs ! no_runs = no_runs + 1. Tell how many times run de program.
...
REAL*8,  DIMENSION(:), ALLOCATABLE ::  media_du_sobre_dt_previous
REAL*8,  DIMENSION(:), ALLOCATABLE ::  sup_du_sobre_dt_previous
...
REAL*8,  DIMENSION(:), ALLOCATABLE ::  media_du_sobre_dt_LOG_previous
REAL*8,  DIMENSION(:), ALLOCATABLE ::  sup_du_sobre_dt_LOG_previous
...
Do numLoops = 1, 2000
    ...
    ALLOCATE( media_du_sobre_dt_LOG_previous(0:no_runs) )
    ALLOCATE( sup_du_sobre_dt_LOG_previous (0:no_runs) )
    ...
    DO i = 0, no_runs
        read(10,520) media_du_sobre_dt_LOG_previous(i)
    ENDDO

    DO i = 0, no_runs
        read(10,520) sup_du_sobre_dt_LOG_previous(i)
    ENDDO
    ...
    ALLOCATE( media_du_sobre_dt_previous(0:previous_length) )
    ALLOCATE( sup_du_sobre_dt_previous (0:previous_length) )
    !**************************************************************************
    ...
    read(10,520)
    DO i = 0, previous_length
        read(10, 520) media_du_sobre_dt_previous(i), sup_du_sobre_dt_previous(i)
    ENDDO
    ...
    DO i = 0, no_runs
        write(20,520) media_du_sobre_dt_LOG_previous(i)
    ENDDO

    DO i = 0, no_runs
        write(20,520) sup_du_sobre_dt_LOG_previous(i)
    ENDDO
    ...
    write(20,520)
    DO i = 0, previous_length
        write(20,520) media_du_sobre_dt_previous(i), sup_du_sobre_dt_previous(i)
    ENDDO
    ...
    if ( tF <  2.1D0  ) tF_new =   5D0;
    if ( tF <  1.1D0  ) tF_new =   2D0;
    if ( tF <  0.6D0  ) tF_new =   1D0;
    ...
    ! dt_dump = 0.01D0   ! time between consecutive dumpings
    ...
    ALLOCATE( media_du_sobre_dt(1:no_dumps) )
    ALLOCATE( sup_du_sobre_dt (1:no_dumps) )
    ...
    DO WHILE ( t < tF_check )
        ...
        DO WHILE ( t < time_next_dump - dt/2)
        ...
        ENDDO  ! DO WHILE ( t < time_next_dump )
        ...
        ! computing mean value of du/dt on the grid

        media_du_sobre_dt(new_count) = (sum(u - u_previous)/dt)/((M2-M1+1)*(N2-N1+1))

        ! computing the maximun absolut value of du/dt on the grid

        sup_du_sobre_dt(new_count) = maxval(abs(u - u_previous))/dt;
    ...
    DO i = 0, no_runs-1
        write(50,540) media_du_sobre_dt_LOG_previous(i)
    ENDDO
     write(50,540) media_du_sobre_dt(new_count)

    DO i = 0, no_runs-1
        write(50,540) sup_du_sobre_dt_LOG_previous(i)
    ENDDO
     write(50,540) sup_du_sobre_dt(new_count)
    ...
    write(50,540)
    DO i = 0, previous_length
        write(50,540) media_du_sobre_dt_previous(i), sup_du_sobre_dt_previous(i)
    ENDDO
    DO i = 1, new_count
        write(50,540) media_du_sobre_dt(i), sup_du_sobre_dt(i)
    ENDDO
    ...
    write(70,*) " M1 = ", M1, "   M2 = ", M2
    write(70,*) " N1 = ", N1, "   N2 = ", N2
    write(70,*)
    write(70,*) " x_min = ", x_min, "   x_max = ", x_max
    write(70,*) " y_min = ", y_min, "   y_max = ", y_max
    write(70,*)
    write(70,*) " refv1 = ", refv1
    write(70,*) " refv2 = ", refv2
    write(70,*) " refv3 = ", refv3
    write(70,*)
    ...
    write(70,*) " h = ", h
    write(70,*) "dt = ", dt
    write(70,*) "dt_dump = ", dt_dump
    write(70,*) "cfl = ", cfl
    write(70,*)
    write(70,*) "Singularities:"
    write(70,*) " x1 = ", x1, "   y1 = ", y1
    write(70,*) " x2 = ", x2, "   y2 = ", y2
    write(70,*) " x3 = ", x3, "   y3 = ", y3
    write(70,*) " b1 = ", b1
    write(70,*) " b2 = ", b2
    write(70,*) " b3 = ", b3
    write(70,*)
    write(70,*) "value of b used: ", b
    write(70,*) "expected value of b: ", (b1 + b2 + b3)/3D0
    write(70,*)
    write(70,*) "current value of tF: ", tF
    write(70,*) "quantidade de atualizacoes total de u desde t=0: ", Qt_Atualizacoes_u
    write(70,*) "Min value of u:", minval([minval(min_u(1:new_count)),minval(min_u_previous(0:previous_length))])
    write(70,*) "Max value of u:", maxval([maxval(max_u(1:new_count)),maxval(max_u_previous(0:previous_length))])
    write(70,*) '***************************************************************************'
    write(70,*)
    write(70,*)
    write(70,*)
    ...
    do i = 0, no_runs-1
        write(70,725) i, variation_mass_LOG_previous(i), variation_sup_LOG_previous(i), &
            media_du_sobre_dt_LOG_previous(i), sup_du_sobre_dt_LOG_previous(i)
        write(70,721)
    enddo
    i = no_runs;
    write(70,725) i, variation_mass(new_count), variation_sup(new_count), &
        media_du_sobre_dt(new_count), sup_du_sobre_dt(new_count)
    write(70,721)
    ...
    DEALLOCATE( media_du_sobre_dt )
    DEALLOCATE( sup_du_sobre_dt )
    ...
    DEALLOCATE( media_du_sobre_dt_previous )
    DEALLOCATE( sup_du_sobre_dt_previous )
    ...
    DEALLOCATE( media_du_sobre_dt_LOG_previous )
    DEALLOCATE( sup_du_sobre_dt_LOG_previous )

    !-----------------------------------------
    !       Deallocation completed
    !-----------------------------------------
end do ! do numLoops = 1, 2000 linha 171

END PROGRAM u24h_serial
